...
function fixture(context, options = {}) {
  ...
  for (const path of [tools, home, temporary, source, destination])
    mkdirSync(path, { recursive: true });
  for (const command of ['tar', 'gzip', 'mktemp', 'mkdir', 'chmod', 'mv', 'rm', 'shasum']) {
...
`,
  ...
  const digest = options.corrupt
    ? '0'.repeat(64)
    : createHash('sha256').update(readFileSync(archive)).digest('hex');
  const checksum = join(directory, 'release.sha256');
  writeFileSync(
    checksum,
    `${digest}  ${options.wrongName ? 'wrong.tar.gz' : `cutegantt-${target}.tar.gz`}\n`,
  );
  ...
  const run = (args = [], piped = false) =>
    spawnSync('/bin/sh', piped ? ['-s', '--', ...args] : [installer, ...args], {
      encoding: 'utf8',
      timeout: 15000,
...
for (const [os, arch, target] of [
  ...
  test(`installer selects ${target}, pins latest and replaces an existing binary`, (context) => {
    ...
    const requests = readFileSync(setup.log, 'utf8')
      .trim()
      .split('\n')
...
for (const [name, options, args, message] of [
  ...
  [
    'unexpected latest redirect',
    { latest: 'https://example.com/v1.2.3' },
    [],
    /did not return a release tag/,
  ],
